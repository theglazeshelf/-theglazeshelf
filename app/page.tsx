...
export default function App(){
 ...
 useEffect(()=>{if(!showLoginSpin)return;const timer=window.setTimeout(()=>setShowLoginSpin(false),1400);return()=>window.clearTimeout(timer)},[showLoginSpin]);
 ...
 async function auth(signup=false){const r=signup?await sb.auth.signUp({email,password}):await sb.auth.signInWithPassword({email,password});if(r.error)setMsg(r.error.message);else if(signup)setMsg("Account created. Check email if required.");else setShowLoginSpin(true)}
 ...
 async function search(quick=false){setSearching(true);setSearchStarted(true);const r=kind==="glaze"?await sb.rpc("find_glazes",{p_query:q.trim()||null,p_cone:quick?null:(finderCone?Number(finderCone):null),p_color_family:quick?null:(colorSearch||null),p_effect:quick?null:(effectSearch.trim()||null),p_user_id:session.user.id,p_studio_id:studio||null,p_access:searchScope,p_limit:60}):await sb.rpc("search_clays",{p_query:q.trim()||null,p_cone:null,p_limit:40});setSearching(false);if(r.error)setMsg(r.error.message);else{setMsg("");setResults(r.data??[])}}
 ...
 async function mine(x:any){const r=kind==="glaze"?await sb.rpc("set_my_glaze",{p_glaze_id:x.glaze_id,p_status:"owned",p_quantity:null,p_notes:null}):await sb.rpc("set_my_clay",{p_clay_id:x.clay_id,p_status:"owned",p_notes:null});if(r.error)setMsg(r.error.message);else{setMsg(`Added ${x.glaze_name||x.clay_name||"item"} to My Shelf ✓`);await load()}}
 ...
 if(recovery)return <main className="simple-auth-shell"><section className="simple-auth-wrap"><img className="simple-auth-logo" src={purpleLogo.src} alt="The Glaze Shelf"/><h1>Create a new password</h1><p className="simple-auth-help">Choose a password with at least 8 characters.</p><div className="stack simple-auth-form"><input className="input" type="password" autoComplete="new-password" placeholder="New password" value={newPassword} onChange={e=>setNewPassword(e.target.value)}/><input className="input" type="password" autoComplete="new-password" placeholder="Confirm new password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)}/><button className="btn simple-auth-primary" onClick={saveNewPassword}>Save New Password</button><button className="btn ghost simple-auth-create" onClick={()=>{sb.auth.signOut();setRecovery(false);setMsg("")}}>Back to Sign In</button>{msg&&<div className="notice">{msg}</div>}</div></section></main>;
 if(!session)return <main className="simple-auth-shell"><section className="simple-auth-wrap"><img className="simple-auth-logo" src={purpleLogo.src} alt="The Glaze Shelf"/><h1>Sign in</h1><form className="stack simple-auth-form" onSubmit={e=>{e.preventDefault();auth()}}><label className="auth-field">Email<input className="input" type="email" autoComplete="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)}/></label><label className="auth-field">Password<input className="input" type="password" autoComplete="current-password" placeholder="Your password" value={password} onChange={e=>setPassword(e.target.value)}/></label><button className="forgot-link" type="button" onClick={forgotPassword}>Forgot password?</button><button className="btn simple-auth-primary" type="submit">Sign In <ArrowRight size={18}/></button><div className="auth-divider"><span>New here?</span></div><button className="btn ghost simple-auth-create" type="button" onClick={()=>auth(true)}>Create an Account</button>{msg&&<div className="notice">{msg}</div>}</form></section></main>;
 return <>{showLoginSpin&&<div className="pottery-login-intro" aria-hidden="true"><div className="pottery-wheel-ring"><div className="pottery-wheel-cup"><img src={purpleLogo.src} alt=""/></div></div></div>}<main className={"shell "+(tab==="home"?"home-shell":"")}>{tab!=="home"&&<div className="row app-header logo-free-header"><button className="nav" aria-label="Sign out" onClick={()=>sb.auth.signOut()}><LogOut size={20}/></button></div>}
 {tab==="home"&&<>
 <section className="home-dashboard-hero"><img className="home-final-logo" src={purpleLogo.src} alt="The Glaze Shelf"/><button className="home-signout" aria-label="Sign out" onClick={()=>sb.auth.signOut()}><LogOut size={20}/></button></section>
 <div className="home-aubergine-content"><section className="home-welcome"><h1>Ready for your next glaze test?</h1><p>Build it. Fire it. Learn from it.</p></section>
 <div className="home-studio-status"><span>STUDIO</span><div className="row"><strong>{currentStudio?.name||"No studio connected yet"}</strong>{currentStudio&&<button className="home-studio-open" onClick={()=>openStudio(currentStudio.studio_id)}>Open Shelf <ArrowRight size={16}/></button>}</div></div>
 <details className="card studio-tools compact-studio-tools home-studio-add"><summary>{currentStudio?"+ Create or Join Another Studio":"+ Create or Join a Studio"}</summary><div className="stack studio-form">{currentStudio?.role==="owner"&&<><button className="btn ghost" onClick={()=>invite(currentStudio.studio_id)}>Invite Someone to {currentStudio.name}</button><div className="studio-divider"><span>or add another</span></div></>}<label className="field-label">Studio Name<input className="input" placeholder="e.g., My Studio" value={studioName} onChange={e=>setStudioName(e.target.value)}/></label><button className="btn secondary" onClick={createStudio}>Create Studio</button><div className="studio-divider"><span>or</span></div><label className="field-label">Invite Code<input className="input" placeholder="Enter invite code" value={join} onChange={e=>setJoin(e.target.value)}/></label><button className="btn ghost" onClick={joinStudio}>Join Studio</button></div></details>
 <div className="quick-action-grid"><button className="quick-action build" onClick={()=>setTab("build")}><span className="quick-icon"><Layers size={21}/></span><span><strong>Build a Combination</strong><small>Layer glazes and plan your piece</small></span><ArrowRight size={18}/></button><button className="quick-action find" onClick={()=>openFinder("all")}><span className="quick-icon"><Search size={21}/></span><span><strong>Find by Effect</strong><small>Search color, movement, and finish</small></span><ArrowRight size={18}/></button><button className="quick-action shelf" onClick={()=>{setShelfView("materials");setTab("shelf")}}><span className="quick-icon"><Library size={21}/></span><span><strong>Open My Shelf</strong><small>{myGlazeCount} {myGlazeCount===1?"glaze":"glazes"} saved</small></span><ArrowRight size={18}/></button><button className="quick-action journal" onClick={()=>setTab("journal")}><span className="quick-icon"><NotebookPen size={21}/></span><span><strong>Firing Journal</strong><small>{firings.length} {firings.length===1?"firing":"firings"} recorded</small></span><ArrowRight size={18}/></button></div>
 {currentStudio&&studios.length>1&&<div className="card"><strong>Your Other Studios</strong>{studios.filter(s=>s.studio_id!==currentStudio.studio_id).map(s=><button className="studio-list-item" key={s.studio_id} onClick={()=>setStudio(s.studio_id)}><span><strong>{s.name}</strong><small>{s.role==="owner"?"You created this studio":"Member"}</small></span><span>Choose →</span></button>)}</div>}</div>
 </>}
