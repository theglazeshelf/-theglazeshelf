...
import {Home,Search,Library,Layers,NotebookPen,LogOut,Plus,ArrowRight} from "lucide-react";
...
export default function App(){
 ...
 function addShelfGlazeToBuild(x:any){setLayers([...layers,{glaze_id:x.item_id,glaze_name:x.item_name,manufacturer:x.manufacturer,coats:2,surface:"inside & outside",placement:"overall",placementMode:"preset"}]);setAnalysis(null);setMsg(`${x.item_name} added to your build ✓`);setTab("build")}
 ...
 if(!session)return <main className="shell"><div className="auth"><img className="auth-logo" src={tealLogo.src} alt="The Glaze Shelf"/><p>Know what you have. Discover what works.</p><div className="stack"><input className="input" type="email" autoComplete="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)}/><input className="input" type="password" autoComplete="current-password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)}/><button className="forgot-link" onClick={forgotPassword}>Forgot password?</button><div className="grid"><button className="btn primary" onClick={()=>auth()}>Sign in</button><button className="btn ghost" onClick={()=>auth(true)}>Create account</button></div>{msg&&<div className="notice">{msg}</div>}</div></div></main>;
 ...
 {tab==="home"&&<><section className="hero home-hero"><div className="home-logo-color"><img src={logoSuite.src} alt="The Glaze Shelf"/></div><p>Build it. Fire it. Learn from it.</p></section>
 {currentStudio?<>
