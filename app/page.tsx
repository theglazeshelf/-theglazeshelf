...
export default function App(){
 ...
 return <main className={"shell "+(tab==="home"?"home-shell":"")}>{tab!=="home"&&<div className="row app-header"><div className="brand"><img className="header-final-logo" src={purpleLogo.src} alt="The Glaze Shelf"/></div><button className="nav" aria-label="Sign out" onClick={()=>sb.auth.signOut()}><LogOut size={18}/></button></div>}
 {tab==="home"&&<>
 ...
 {currentStudio?<div className="card current-studio-card home-studio-first"><span className="eyebrow">YOUR CURRENT STUDIO</span><div className="row studio-heading"><div><strong className="studio-name">{currentStudio.name}</strong><div className="muted">{currentStudio.role==="owner"?"You created this studio":"You’re a member of this studio"}</div></div><span className="tag">Current</span></div><div className="grid"><button className="btn primary" onClick={()=>openStudio(currentStudio.studio_id)}>Open Studio Shelf</button>{currentStudio.role==="owner"&&<button className="btn ghost" onClick={()=>invite(currentStudio.studio_id)}>Invite Someone</button>}</div></div>:<><div className="card studio-empty compact-studio-status home-studio-first"><div className="row"><div><span className="eyebrow">STUDIO</span><strong>No studio connected yet</strong></div><span className="tag">Set up</span></div></div><details className="card studio-tools compact-studio-tools"><summary>+ Create or Join a Studio</summary><div className="stack studio-form"><label className="field-label">Studio Name<input className="input" placeholder="e.g., My Studio" value={studioName} onChange={e=>setStudioName(e.target.value)}/></label><button className="btn secondary" onClick={createStudio}>Create Studio</button><div className="studio-divider"><span>or</span></div><label className="field-label">Invite Code<input className="input" placeholder="Enter invite code" value={join} onChange={e=>setJoin(e.target.value)}/></label><button className="btn ghost" onClick={joinStudio}>Join Studio</button></div></details></>}
 ...
 <div className="dashboard-stats"><div><strong>{myGlazeCount}</strong><span>My Glazes</span></div><div><strong>{recipes.length}</strong><span>Recipes</span></div><div><strong>{studios.length}</strong><span>{studios.length===1?"Studio":"Studios"}</span></div></div>
 {currentStudio&&<>{studios.length>1&&<div className="card"><strong>Your Other Studios</strong>{studios.filter(s=>s.studio_id!==currentStudio.studio_id).map(s=><button className="studio-list-item" key={s.studio_id} onClick={()=>setStudio(s.studio_id)}><span><strong>{s.name}</strong><small>{s.role==="owner"?"You created this studio":"Member"}</small></span><span>Choose →</span></button>)}</div>}<details className="card studio-tools"><summary>Create or join another studio</summary><div className="stack studio-form"><label className="field-label">Studio Name<input className="input" placeholder="e.g., My Studio" value={studioName} onChange={e=>setStudioName(e.target.value)}/></label><button className="btn secondary" onClick={createStudio}>Create Studio</button><div className="studio-divider"><span>or</span></div><label className="field-label">Invite Code<input className="input" placeholder="Enter invite code" value={join} onChange={e=>setJoin(e.target.value)}/></label><button className="btn ghost" onClick={joinStudio}>Join Studio</button></div></details></>}</div>
 </>}
