...
export default function App(){
 ...
 async function saveRecipe(){const name=recipeName.trim()||layers.map(x=>x.glaze_name).join(" + ")||"Saved Combination";const r=await sb.rpc("save_recipe_from_stack",{p_name:name,p_clay_id:clay?.clay_id??null,p_cone:cone,p_form:"vertical",p_texture:"carved",p_goal:goal||null,p_glaze_ids:layers.map(x=>x.glaze_id),p_coats:layers.map(x=>x.coats),p_placements:layers.map(x=>x.placement||"overall")});if(r.error)setMsg(r.error.message);else{setRecipeName("");setMsg("Recipe saved ✓");await load();setShelfView("recipes");setTab("shelf")}}
 ...
 function editRecipe(){if(!recipeDetail.length)return;const first=recipeDetail[0];setClay(first.clay_id?{clay_id:first.clay_id,clay_name:first.clay_name}:null);setCone(Number(first.cone)||6);setGoal(first.goal||"");setRecipeName(first.recipe_name||"");setLayers(recipeDetail.map(x=>({glaze_id:x.glaze_id,glaze_name:x.glaze_name,manufacturer:x.manufacturer,coats:Number(x.coats)||2,placement:x.placement||"overall"})));setAnalysis(null);setRecipeDetail([]);setTab("build")}
 ...
 {tab==="home"&&<><section className="hero home-hero"><img className="home-logo" src={tealLogo.src} alt="The Glaze Shelf"/><h1>What are you glazing today?</h1><p>Build it. Fire it. Learn from it.</p></section><div className="card"><strong>Studios</strong>{studios.map(s=><div className="item" key={s.studio_id}><div className="row"><strong>{s.name}</strong><span className="tag">{s.role}</span></div><div className="grid"><button className="btn secondary" onClick={()=>openStudio(s.studio_id)}>Open Shelf</button>{s.role==="owner"&&<button className="btn ghost" onClick={()=>invite(s.studio_id)}>Invite</button>}</div></div>)}<div className="stack"><input className="input" placeholder="New studio" value={studioName} onChange={e=>setStudioName(e.target.value)}/><button className="btn secondary" onClick={createStudio}>Create Studio</button><input className="input" placeholder="Invite code" value={join} onChange={e=>setJoin(e.target.value)}/><button className="btn ghost" onClick={joinStudio}>Join Studio</button></div></div>{studio&&<div className="card"><strong>Studio Shelf</strong>{studioShelf.map(x=><div className="item" key={x.item_type+x.item_id}>{x.item_name}<div className="muted">{x.manufacturer} • {x.item_type}</div></div>)}</div>}</>}
...
{tab==="build"&&<>
      ...
      <div className="row">
        <div>
          ...
          <div className="layer-controls">
            ...
            <label>Placement
              <select className="mini-select" value={x.placement||"overall"} onChange={e=>{const next=[...layers];next[i]={...next[i],placement:e.target.value};setLayers(next);setAnalysis(null)}}>
                <option value="overall">Overall</option><option value="top half">Top half</option><option value="rim">Rim</option><option value="design only">Design only</option><option value="inside">Inside</option><option value="outside">Outside</option>
              </select>
            </label>
          </div>
        </div>
 ...
 {recipeDetail.length>0&&<div className="overlay" onClick={()=>setRecipeDetail([])}><div className="recipe-sheet" onClick={e=>e.stopPropagation()}><div className="row"><div><span className="eyebrow">SAVED RECIPE</span><h2>{recipeDetail[0].recipe_name}</h2></div><button className="close" aria-label="Close recipe" onClick={()=>setRecipeDetail([])}>×</button></div><div className="recipe-meta"><span>{recipeDetail[0].clay_name||"No clay selected"}</span><span>Cone {recipeDetail[0].cone}</span><span>{recipeDetail[0].texture||"No texture"}</span></div>{recipeDetail[0].goal&&<div className="goal"><span className="eyebrow">DESIRED EFFECT</span><p>{recipeDetail[0].goal}</p></div>}<div className="recipe-layers">{recipeDetail.map((x,i)=><div className="recipe-layer" key={x.glaze_id+i}><span className="layer-number">{i+1}</span><div><span className="muted">{i===0?"Base glaze":`Layer ${i+1}`}</span><strong>{x.glaze_name}</strong><span className="muted">{x.manufacturer}</span></div><div className="layer-detail"><strong>{x.coats||"—"}</strong><span className="muted">coats</span><span>{x.placement||"overall"}</span></div></div>)}</div><div className="stack"><button className="btn primary" onClick={()=>startFiring(recipeDetail[0].recipe_id,recipeDetail[0].cone)}>Start Firing Log</button><button className="btn ghost" onClick={editRecipe}>Edit coats in Builder</button></div></div></div>}
