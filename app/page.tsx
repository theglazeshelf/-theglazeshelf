...
export default function App(){
 ...
 if(!session)return <main className="auth-shell"><div className="auth-glow auth-glow-one"/><div className="auth-glow auth-glow-two"/><div className="auth-glow auth-glow-three"/><section className="auth-brand-panel"><div className="color-logo-crop auth-color-logo"><img src={logoSuite.src} alt="The Glaze Shelf"/></div><h1>Know what you have.<br/>Discover what works.</h1></section><section className="auth-form-card"><span className="eyebrow">WELCOME TO THE GLAZE SHELF</span><h2>Sign in to your shelf</h2><form className="stack" onSubmit={e=>{e.preventDefault();auth()}}><label className="auth-field">Email<input className="input" type="email" autoComplete="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)}/></label><label className="auth-field">Password<input className="input" type="password" autoComplete="current-password" placeholder="Your password" value={password} onChange={e=>setPassword(e.target.value)}/></label><button className="forgot-link" type="button" onClick={forgotPassword}>Forgot password?</button><button className="btn primary auth-signin" type="submit">Sign In <ArrowRight size={18}/></button><div className="auth-divider"><span>New to The Glaze Shelf?</span></div><button className="btn ghost auth-create" type="button" onClick={()=>auth(true)}>Create an Account</button>{msg&&<div className="notice">{msg}</div>}</form></section><section className="auth-details"><p>Organize your glazes, build combinations, and learn from every firing.</p><div className="auth-features"><span><Library size={16}/>Your shelf</span><span><Layers size={16}/>Build recipes</span><span><NotebookPen size={16}/>Track results</span></div></section></main>;
 ...
 {tab==="home"&&<>
 <section className="home-dashboard-hero"><div className="color-logo-crop home-color-logo"><img src={logoSuite.src} alt="The Glaze Shelf"/></div><h1>Ready for your next glaze test?</h1><p>Build it. Fire it. Learn from it.</p></section>
