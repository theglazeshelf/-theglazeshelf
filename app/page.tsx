...
export default function App(){
 ...
 function addShelfGlazeToBuild(x:any){const glazeName=x.item_name||x.name,manufacturer=typeof x.manufacturer==="string"?x.manufacturer:x.manufacturer?.name;setLayers([...layers,{glaze_id:x.item_id||x.id,glaze_name:glazeName,manufacturer,coats:2,surface:"inside & outside",placement:"overall",placementMode:"preset"}]);setAnalysis(null);setGlazeDetail(null);setMsg(`${glazeName} added to your build ✓`);setTab("build")}
 async function openGlazeDetail(x:any){setGlazeDetail({id:x.item_id,name:x.item_name,manufacturer:{name:x.manufacturer}});setGlazeDetailLoading(true);const r=await sb.from("glazes").select("id,name,sku,cone_min,cone_max,finish,opacity,movement_score,food_contact_note,notes,manufacturer:manufacturers(name),glaze_line:glaze_lines(name,cone_min,cone_max),intelligence:glaze_intelligence(manufacturer_movement,our_type,behavior_score,color_family,breaks_over_texture,pools,waterfall_potential,layer_reactivity,run_risk,layer_role,cone_text,dinnerware_safe,manufacturer_description,best_pairings,studio_notes,confidence,last_verified,food_safe_chemistry_claim,food_contact_surface_recommended,food_contact_restriction_reason)").eq("id",x.item_id).single();setGlazeDetailLoading(false);if(r.error){setGlazeDetail(null);setMsg(r.error.message)}else setGlazeDetail(r.data)}
